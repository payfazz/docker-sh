...
# this is forked from https://github.com/payfazz/sh-script/blob/b0bd0349b25846ebbe9584b203c444bf513b433c/bin/docker-work
# BEGIN docker-work
define_user=1
randomname="work-$(_random)"
uid=$(id -u)
gid=$(id -g)
...
for arg in "$@"; do
  case "$arg" in
    ...
    --name|-h*|--hostname|--hostname=*) panic "$arg is not supported";;
  esac
done
...
if [ "$define_user" = 1 ]; then
  docker_cmds="$docker_cmds -u '$uid:$gid'"
  ( (
    count=0
    while :; do
...
docker_cmds="$(quote "$docker_cmds") $(no_proc=y quote "$@")" || panic
eval "set -- $docker_cmds"
"$@"
exit 1
# END docker-work
